    ...
    Freezable
{
    ...
    function _update(address from, address to, uint256 amount) internal virtual override whenNotPaused {
        if (from != address(0) && isBlocked(from)) revert BlockedSender(from);
        if (to != address(0) && isBlocked(to)) revert BlockedRecipient(to);
    ...
    function transferFromReserve(address to, uint256 amount) external onlyRole(MINTER_ROLE) {
    ...
    function setTimelock(address payable _timelock) external onlyRole(DEFAULT_ADMIN_ROLE) {
